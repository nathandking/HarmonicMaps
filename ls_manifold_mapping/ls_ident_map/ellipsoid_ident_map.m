%% Computation of identity map for a ellipsoid from an initial noisy map.
...
%% banding: do calculation in a narrow band around the ellipsoid
dim = 3;    % dimension
...
for j = 1:num_rep
    for i = 1:length(dx)    
        ...
        % function cpEllipsoid for finding the closest points on ellipsoid
        [cpx, cpy, cpz, dist] = cpEllipsoid(xx,yy,zz);
        % make into vectors
        cpxg = cpx(:); cpyg = cpy(:); cpzg = cpz(:);

        % compute band
        % NOTE: if CPM bandwidth is used convergence order is bad. Using 5
        % times CPM bandwidth gave roughly 0.6 convergence order for first
        % 3 dx values. Using the larger bandwidth takes much longer however
        % since more points are used and the initial hypercube must be
        % larger. Error returned is more than that of the error when
        % using the CPM bandwidth.
        ...
        % map noisy data back onto ellipsoid and assign initial u1, u2, u3
        [u1, u2, u3] = cpEllipsoid(Ncp(:,1), Ncp(:,2), Ncp(:,3));
        ...
        for kt = 1:numtimesteps
            ...
            % project onto ellipsoid N
%            [~,~,~,sd_new] = cpEllipsoid(u1, u2, u3);
             sd_new = Fx(u1, u2, u3);
